  ...
  return {
    ...
    load: async function (this: Loader, { store, meta, logger }) {
      ...
      try {
        ...
        if (type === "collections" && (!posts || !Array.isArray(posts))) {
        ...
        else store.set({ id: posts.id, data: posts });
    ...
    schema: async () => {
      const urlForSchema = `/get-strapi-schema/schema/${contentType}`;

      const data = await fetchFromStrapi(urlForSchema);
      if (!data?.attributes) {
